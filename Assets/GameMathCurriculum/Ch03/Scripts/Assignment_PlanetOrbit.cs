...
public class Assignment_PlanetOrbit : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        Quaternion rotation = Quaternion.Euler(0f, planetAngle, 0f);
        Vector3 sclae = new Vector3(1f, 1f, 1f);

        Matrix4x4 m = Matrix4x4.TRS(orbitCenter, rotation, sclae);

        transform.position = m.MultiplyPoint(new Vector3(planetOrbitRadius, 1f, 0f));

        float satelliteAngle = Time.time * satelliteOrbitSpeed;

        Quaternion satelliteRotation = Quaternion.Euler(0f, satelliteAngle, 0f);
        Vector3 satelliteSclae = new Vector3(1f, 1f, 1f);

        Matrix4x4 m2 = Matrix4x4.TRS(transform.position, satelliteRotation, satelliteSclae);

        satellite.position = m2.MultiplyPoint(new Vector3(satelliteOrbitRadius, 0f, 0f));
