...
public class Assignment_GachaSimulator : MonoBehaviour
{
    ...
    private void ExecutePull()
    {
        ...
        {
            var increment = (1f - baseRate) / (hardPity - softPityStart);
            currentEffectiveRate += increment;
        }
        else
        {
        ...
        if(value < currentEffectiveRate)
        {
            currentPityCount++;
            pullHistory.Add(true);
            if(pullHistory.Count > MAX_HISTORY)
            {
                pullHistory.Remove(pullHistory.First());
            }
            totalSSRs++;
            ssrPityList.Add(currentPityCount);
            if(ssrPityList.Count > 3)
            {
                ssrPityList.Remove(ssrPityList.First());
            }
            currentPityCount = 0;
        }
        else
